...
export default function CumulativeSpendingChart({ data }: Props) {
  ...
  const cumulative = data.points.map((p) => p.cumulative_total)
  const monthly = data.points.map((p) => p.monthly_total)
      ...
      data={[
        {
          ...
          customdata: monthly,
          hovertemplate: '%{x}<br>Cumulative: $%{y:,.2f}<br>Monthly: $%{customdata:,.2f}<extra></extra>',
        },
      ]}
      layout={{
